...
- **The dashboard views exactly one bank.** There is no combined view: `parseBankFromSearch()` never returns null, falling back to `DEFAULT_BANK` (TPBank) when `?bank=` is absent or unrecognised. `availableBanks` on the `AggregatedView` is still computed *before* the filter, so the dropdown keeps offering the other banks in the period.
