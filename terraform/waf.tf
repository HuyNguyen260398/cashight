...
# Same ruleset as the CloudFront ACL above; REGIONAL scope runs in ap-southeast-1
# so no provider alias is needed.
...
resource "aws_wafv2_web_acl" "api" {
  ...
  rule {
    ...
    visibility_config {
      cloudwatch_metrics_enabled = true
      metric_name                = "${var.project_name}-api-known-bad-inputs"
      sampled_requests_enabled   = true
    }
  }
  ...
  rule {
    ...
    visibility_config {
      cloudwatch_metrics_enabled = true
      metric_name                = "${var.project_name}-api-ip-reputation"
      sampled_requests_enabled   = true
    }
  }
